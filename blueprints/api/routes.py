...
def api_get_profile(uid: str):
    ...
    logger.info("GET /profile called for uid: %s", uid)
    profile_data = get_profile_data(uid)
    logger.info("GET /profile success for uid: %s", uid)
...
def api_create_profile(uid: str):
    ...
    logger.info("POST /profile called for uid: %s", uid)
    content_error = require_json_content_type()
    if content_error:
        logger.info("POST /profile bad content type for uid: %s", uid)
    ...
    if error:
        logger.info("POST /profile validation error for uid: %s - %s", uid, error)
        return jsonify({"error": error}), 400

    normalized = normalize_profile_data(first_name, last_name, student_id)
    set_profile(uid, normalized, merge=False)
    logger.info("POST /profile saved successfully for uid: %s", uid)
...
def api_update_profile(uid: str):
    ...
    logger.info("PUT /profile called for uid: %s", uid)
    content_error = require_json_content_type()
    if content_error:
        return content_error
    ...
    if errors:
        logger.info("PUT /profile validation errors for uid: %s - %s", uid, errors)
    ...
    logger.info("PUT /profile updated successfully for uid: %s", uid)
...
def api_delete_profile(uid: str):
    ...
    logger.info("DELETE /profile called for uid: %s", uid)
    get_profile_doc_ref(uid).delete()
    logger.info("DELETE /profile success for uid: %s", uid)
...
def api_get_sensor_data(uid: str):
    ...
    logger.info("GET /sensor_data called for uid: %s", uid)
    _ = uid
    ...
    try:
        ...
        logger.info("GET /sensor_data: mock_sensor_data.json not found")
        return jsonify({"error": "mock_sensor_data.json not found"}), 404
    except json.JSONDecodeError:
        logger.info("GET /sensor_data: mock_sensor_data.json invalid JSON")
        return jsonify({"error": "mock_sensor_data.json is not valid JSON"}), 500

    logger.info("GET /sensor_data success for uid: %s", uid)
...
def api_sensor_data():
    ...
    logger.info("POST /sensor_data called")
    content_error = require_json_content_type()
    if content_error:
        return content_error
    ...
    logger.info("POST /sensor_data saved with id: %s", doc_id)
